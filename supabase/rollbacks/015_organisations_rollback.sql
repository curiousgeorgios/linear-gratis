...
-- Run via `supabase db execute` or psql only if needed.
...
-- Drop policies on the organisation tables before dropping organisation_members.
-- The organisations policies query organisation_members, so the table drop must
-- stay dependency-clean rather than relying on DROP TABLE ... CASCADE.
DROP POLICY IF EXISTS "Members can view their orgs" ON organisations;
DROP POLICY IF EXISTS "Authenticated users can create orgs" ON organisations;
DROP POLICY IF EXISTS "Owners can update their orgs" ON organisations;
DROP POLICY IF EXISTS "Owners can delete their orgs" ON organisations;
...
ALTER TABLE public_views           DROP COLUMN IF EXISTS organisation_id;
ALTER TABLE customer_request_forms DROP COLUMN IF EXISTS organisation_id;
ALTER TABLE branding_settings      DROP COLUMN IF EXISTS organisation_id;
ALTER TABLE custom_domains         DROP COLUMN IF EXISTS organisation_id;
ALTER TABLE roadmaps               DROP COLUMN IF EXISTS organisation_id;
