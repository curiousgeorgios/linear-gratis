...
- `supabase/rollbacks/015_organisations_rollback.sql` — companion, never auto-applied
...
**Files:**
- Create: `supabase/rollbacks/015_organisations_rollback.sql`

Not registered as a Supabase migration — it lives outside `supabase/migrations` and is run manually only if a critical issue surfaces post-deploy.

- [ ] **Step 1: Write the rollback file**

Create `supabase/rollbacks/015_organisations_rollback.sql`. Keep the full SQL
body in that file rather than duplicating it here, and keep it outside
`supabase/migrations` so `supabase db push` and `supabase db reset` never apply
it as a forward migration.

- [ ] **Step 2: Commit**

```bash
git add supabase/rollbacks/015_organisations_rollback.sql
...
## Rollback
`supabase/rollbacks/015_organisations_rollback.sql` — manual, not auto-applied.
```
